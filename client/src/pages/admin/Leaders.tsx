...
const LeadersPage: React.FC = () => {
  ...
  const columns: Column<Leader>[] = [
    ...
    { id: 'position', label: '职务', width: 180 },
    ...
    },
    {
      ...
      render: (row) => (
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          ...
          <Button size="small" color="error" startIcon={<DeleteIcon />} onClick={() => setDeleteConfirm({ open: true, leader: row })}>删除</Button>
        </Box>
      ),
    },
  ];
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            ...
            <TextField label="职务" value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} required size="small" />
