...
const BannersPage: React.FC = () => {
  ...
  const handleOpenCreate = () => {
    setEditBanner(null);
    setForm({ title: '', imageUrl: '', linkUrl: '', sort: banners.length, isActive: true });
    setDialogOpen(true);
  };
  ...
  const columns: Column<Banner>[] = [
    ...
    { id: 'linkUrl', label: '链接', width: 200, render: (row) => row.linkUrl || '-' },
    ...
    },
    {
      ...
      render: (row) => {
        ...
        <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', alignItems: 'center' }}>
          <IconButton size="small" onClick={() => handleMoveUp(row, idx)} disabled={idx === 0} title="上移">
            <ArrowUpwardIcon fontSize="small" />
          </IconButton>
          <IconButton size="small" onClick={() => handleMoveDown(row, idx)} disabled={idx === banners.length - 1} title="下移">
          ...
          <Button size="small" color="error" startIcon={<DeleteIcon />} onClick={() => setDeleteConfirm({ open: true, banner: row })}>
            删除
          </Button>
        </Box>
        );
